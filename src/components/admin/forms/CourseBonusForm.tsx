...
import { createCourseBonus, updateCourseBonus, deleteCourseBonus } from "@/lib/actions/courseBonuses";
...
interface Props { bonus?: FormData & { id: string }; id?: string; isEdit?: boolean }

export function CourseBonusForm({ bonus, id, isEdit }: Props) {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: {
      text: bonus?.text ?? "",
      displayOrder: bonus?.displayOrder ?? 0,
    },
  });

  async function onSubmit(data: FormData) {
    setSaving(true); setServerError("");
    const res = isEdit && id ? await updateCourseBonus(id, data) : await createCourseBonus(data);
    setSaving(false);
    if (!res.success) { setServerError(res.error ?? "Erro ao salvar"); return; }
    router.push("/admin/course-bonuses");
    router.refresh();
  }

  async function handleDelete() {
    if (!id || !confirm("Excluir este bônus?")) return;
    await deleteCourseBonus(id);
    router.push("/admin/course-bonuses");
    router.refresh();
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 max-w-2xl">
      ...
      <div>
        ...
        <textarea {...register("text")} className={`${inp} resize-none`} rows={3} placeholder="Ex: Acesso vitalício à comunidade fechada" />
